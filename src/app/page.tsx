    ...
    <div className="min-h-screen bg-white text-[#0a0a0a]" style={{ fontFamily: "var(--font-manrope), sans-serif" }}>
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-[#e5e7eb]">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            <div>
              <span className="font-extrabold text-sm tracking-tight block leading-tight">Мой Профсоюз</span>
              <span className="text-[10px] text-[#6b7280] block leading-tight">профсоюзная платформа</span>
            </div>
          </Link>
          <div className="flex items-center gap-3">
            <Link
              href="/login"
              className="hidden sm:inline-flex items-center gap-1.5 text-sm font-semibold text-[#6b7280] hover:text-[#0a0a0a] transition-colors"
            >
              Войти
            </Link>
            <Link
              href="/login"
              className="inline-flex items-center gap-2 rounded-xl bg-[#3b82f6] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#2563eb] transition-colors shadow-sm"
            >
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-20 lg:py-28 text-center">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 rounded-full border border-[#e5e7eb] bg-white px-4 py-1.5 text-xs font-semibold text-[#6b7280] mb-8 shadow-sm">
            <CheckCircle2 className="h-3.5 w-3.5 text-[#3b82f6]" />
            Официальная платформа профсоюза
          </div>

          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight max-w-3xl mx-auto leading-[1.15]">
            Цифровая платформа
            <br />
            <span className="text-[#3b82f6]">вашего профсоюза</span>
          </h1>

          <p className="mt-5 text-base sm:text-lg text-[#6b7280] max-w-xl mx-auto leading-relaxed font-medium">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-10">
            <Link
              href="/login"
              className="inline-flex items-center gap-2.5 rounded-xl bg-[#3b82f6] px-8 py-3.5 text-sm font-bold text-white hover:bg-[#2563eb] transition-colors shadow-md shadow-[#3b82f6]/20"
            >
              Войти в платформу
            ...
            <Link
              href="/register"
              className="inline-flex items-center gap-2 rounded-xl border border-[#e5e7eb] px-6 py-3.5 text-sm font-bold text-[#374151] hover:bg-[#f9fafb] transition-colors"
            >
      ...
      <section className="py-16 lg:py-20 bg-[#f9fafb]">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="mt-3 text-sm text-[#6b7280] font-medium max-w-md mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {features.map((f) => (
              ...
              >
                ...
                <h3 className="text-sm font-extrabold mb-1">{f.title}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            {steps.map((s) => (
              <div key={s.num} className="text-center relative">
                ...
                <h3 className="text-sm font-extrabold mb-1.5">{s.title}</h3>
                <p className="text-xs text-[#6b7280] leading-relaxed font-medium max-w-[260px] mx-auto">
                  {s.description}
                </p>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {trust.map((t) => (
              <div key={t.title} className="flex items-start gap-4 rounded-2xl bg-white border border-[#e5e7eb] p-5">
                ...
                <div>
                  <h3 className="text-sm font-extrabold mb-1">{t.title}</h3>
                  <p className="text-xs text-[#6b7280] leading-relaxed font-medium">{t.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-16 lg:py-20">
        <div className="max-w-6xl mx-auto px-6">
          <div className="rounded-3xl bg-gradient-to-br from-[#3b82f6]/[0.06] to-[#3b82f6]/[0.02] border border-[#3b82f6]/10 px-8 py-14 lg:py-16 text-center">
            ...
            <p className="text-sm text-[#6b7280] font-medium mb-8 max-w-md mx-auto">
      ...
      <footer className="border-t border-[#e5e7eb] py-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-[#9ca3af] font-medium">
