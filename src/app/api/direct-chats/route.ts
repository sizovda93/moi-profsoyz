...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify recipient is in same division
    const { rows: check } = await pool.query(`
      SELECT a1.division_id as my_div, a2.division_id as their_div
      FROM agents a1, agents a2
      WHERE a1.user_id = $1 AND a2.user_id = $2
    `, [user.id, recipientId]);

    if (check.length === 0 || !check[0].my_div || check[0].my_div !== check[0].their_div) {
      return Response.json({ error: "Можно переписываться только с коллегами из вашего подразделения" }, { status: 403 });
    }
