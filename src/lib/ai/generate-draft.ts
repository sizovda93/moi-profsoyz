...
Руководитель общается с членом профсоюза. Тебе нужно предложить черновик ответа руководителя.

Правила:
- Пиши от лица руководителя профсоюза, коротко и по делу (2–4 предложения).
- Обращайся к члену профсоюза по имени, если оно известно.
...
- Если член профсоюза спрашивает о трудовых правах — дай краткую консультацию или направь к юристу через раздел «Вопрос юристу».
- Если обращение по трудовому спору — прими серьёзно, предложи разобраться.
- Если вопрос о профсоюзных мероприятиях, документах или новостях — направь в соответствующий раздел платформы.
- Если жалоба — ответь уважительно, предложи конкретные шаги по решению.
- Если инициатива — поблагодари и подтверди рассмотрение.
- При возможности направь в нужный раздел платформы (О профсоюзе, Вопрос юристу, Обучение, Опросы).
...
export async function generateDraft(
  ...
  const agentName = lastMessage.sender_name || "Участник профсоюза";
  ...
  const contextLines = messages.map((m) => {
    const role = m.sender_type === "agent" ? "Участник профсоюза" : "Руководитель";
...
Имя участника профсоюза: ${agentName}`;
